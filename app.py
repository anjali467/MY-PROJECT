...
def apply():
    ...
    #user_img_pil = Image.open(user_img_path).convert("RGBA")
    #user_img_cv = cv2.cvtColor(np.array(user_img_pil), cv2.COLOR_RGBA2BGR)
    earring_img = Image.open(selected_accessories["Earrings"]).convert("RGBA") if "Earrings" in selected_accessories else None
    necklace_img = Image.open(selected_accessories["Necklace"]).convert("RGBA") if "Necklace" in selected_accessories else None
    purse_img = Image.open(selected_accessories["Purse"]).convert("RGBA") if "Purse" in selected_accessories else None
    ...
    bracelet_img = Image.open(selected_accessories["Bracelet"]).convert("RGBA") if "Bracelet" in selected_accessories else None
    goggle_img = Image.open(selected_accessories["Goggles"]).convert("RGBA") if "Goggles" in selected_accessories else None
    ...
    if bracelet_img:
        ...
        bracelet_pos = (
            ...
            centers["right_wrist"][1] - bracelet_resized.height // 2 + 5
        )
    ...
    #paste goggle image
    if goggle_img:
        # Defensive: use top of nose (NOSE) landmark as center
        ...
        goggle_resized = goggle_img.resize(
            (target_width, int(goggle_img.height * scale_factor)),
            resample=Image.LANCZOS
        )
